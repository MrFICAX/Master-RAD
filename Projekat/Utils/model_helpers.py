...
yaml_settings = general_helpers.load_yaml_settings()
ROOT_DIR = general_helpers.get_root_dir()
MODEL_FOLDER = os.path.join(ROOT_DIR, yaml_settings['MODEL'])
...
def get_open_clip_model(OPEN_CLIP_MODEL: str):
    ...
    model, _, preprocess = open_clip.create_model_and_transforms(model_name, pretrained=pretrained, cache_dir=MODEL_FOLDER)
    model.to(device=get_device())
    return model, preprocess, tokenizer, model_name, pretrained, embedding_size

def get_open_clip_model_and_device(OPEN_CLIP_MODEL: str):
    ...
    model, _, preprocess = open_clip.create_model_and_transforms(model_name, pretrained=pretrained, cache_dir=MODEL_FOLDER)
    model.to(device)
    return model, preprocess, tokenizer, model_name, pretrained, embedding_size, device
